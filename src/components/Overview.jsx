...
import { TrendingUp, Target, Wallet, Zap, Lightbulb, AlertCircle, PiggyBank, Plus, ChevronDown, ChevronUp } from 'lucide-react';

const Overview = ({ categoryData, totalSpent, transactionCount, savingGoal }) => {
  ...
  const insights = [
    {
      ...
      description: `You're spending €${(categoryData.find(c => c.name === 'Food Delivery')?.value || 120).toFixed(2)} on delivery. Cooking at home could save you €72/month!`,
      suggestion: 'Try home-cooked meals twice a week to reduce delivery costs.',
      impact: '€720/year',
      icon: AlertCircle,
    ...
    },
    {
      ...
      description: `You've maintained consistent weekly deposits for four weeks.`,
      suggestion: 'Auto-transfer €20 extra weekly to accelerate your progress.',
    ...
    },
    {
      ...
      description: 'Your essentials account for 62% of total spending — slightly above the healthy 50–60% range.',
      suggestion: 'Redirect 5% from non-essentials to savings next month.',
      impact: '~€120/month',
      icon: Lightbulb,
            ...
            {insights.map((insight, idx) => {
              ...
              return (
                <div
                  ...
                  className={`cursor-pointer bg-gradient-to-br from-slate-800 to-slate-900 border ${insight.border} rounded-2xl p-5 transition-all duration-300 hover:scale-[1.02] hover:shadow-lg`}
                >
                  <div className={`w-10 h-10 bg-gradient-to-br ${insight.color} rounded-lg flex items-center justify-center mb-3`}>
                    <Icon className="w-5 h-5 text-white" />
                  </div>
                  <h3 className="text-base font-bold text-gray-100 mb-2">{insight.title}</h3>
                  <p className="text-gray-400 text-sm mb-3">{insight.description}</p>
                  
                  {isExpanded && (
                    <div className="mt-3 space-y-2 animate-fadeIn">
                      <div className={`bg-gradient-to-br ${insight.color} opacity-80 border border-white/5 rounded-lg p-3`}>
                        <p className="text-sm text-white">
                          💡 <span className="font-semibold">Suggestion:</span> {insight.suggestion}
                        </p>
                      </div>
                      <div className="text-xs text-gray-400">
                        💰 <span className="text-green-400 font-bold">Impact:</span> {insight.impact}
                      </div>
                    </div>
                  )}
                  
                  <button className="text-xs text-green-400 mt-2 flex items-center gap-1">
